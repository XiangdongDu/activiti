...
public class SystemController {
     ...
     * @return
     */
    @ApiOperation("列出所有经理用户")
    @RequestMapping(value = "listManagerUsers/{manager}", method = RequestMethod.GET)
    @ResponseBody
    public List<User> listHRManagerUsers(@PathVariable("manager") String manager) {
        List<User> userlist = systemservice.getallusers();
        ...
        for (User user : userlist) {
            ...
            if (ur != null && ur.size() > 0) {
                for (UserRole userole : ur) {
                    int roleid = userole.getRoleid();
                    Role r = systemservice.getRolebyid(roleid);
                    if (r != null && r.getRolename().equals(manager))
                        users.add(user);
                }
            }
        }
